...
* The client must check response status, which returns any errors or timeouts for each pindex.
...
The query return status is 200 even if all pindexes return errors, so it's critical to check the response status and code accordingly.

* If the client sets timeout low, for example 1{nbsp}ms, you may receive a 200 error with all timeouts instead of a consistency wait timeout.
