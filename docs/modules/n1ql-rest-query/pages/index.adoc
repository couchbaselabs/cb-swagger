...
:page-toclevels: 2
:page-service-desc: _attachments/query-service.bundle.yaml
...
| http (basic)
| xref:security-Header[]



| apiKey
| xref:security-Parameter[]
...
[#get_service-ex-response-400]
.Response 400
====
[#get_service-ex-response-400-0]
.SQL++ Error
[source,json]
----
{
...
----
{
  ...
  "profile" : {
    ...
    "ioTime" : "752.858519ms",
    "~analysis" : [ "High IO time", "High primary scan document count" ],
    "cpuTime" : "90.734075ms",
    "phaseCounts" : {
      "fetch" : 16,
      "indexScan" : 187
  ...
  "generated_statement" : "SELECT country, COUNT(*) AS `airline_count` FROM `travel-sample`.`inventory`.`airline` AS `a` GROUP BY country",
  "metrics" : {
    ...
    "mutationCount" : 5,
    "sortCount" : 2,
    "naturalLanguageProcessingTime" : "naturalLanguageProcessingTime",
    "errorCount" : 9,
...
[#post_service-ex-curl]
= Example HTTP Request

[#post_service-ex-curl-0]
.Form Data
====
...
[#post_service-ex-request]
= Example Request Body


[#post_service-ex-request-0]
.Named Parameters
====
...
[#post_service-ex-request-4]
.Authentication Parameter
====
This request authenticates using a request parameter.
You must use this method of authentication if a request requires more than one set of credentials.
...
ifeval::["yesyesyesyes" != ""]
[#post_service-ex-response]
= Example HTTP Response
endif::[]


[#post_service-ex-response-200]
.Response 200
====
[source,json]
----
...
[#post_service-ex-response-400]
.Response 400
====
[#post_service-ex-response-400-0]
.SQL++ Error
[source,json]
----
{
...
[#post_service-ex-response-400-1]
.Request Error
[source,json]
----
{
...
[%hardbreaks]
xref:Request[]
xref:RequestCredentials[]
xref:Request_natural_config[]
xref:RequestScanVector[]
xref:RequestScanVectorFull[]
xref:RequestScanVectorSparse[]
xref:RequestScanVectorValueGuard[]
xref:RequestScanVectors[]
xref:Response[]
xref:ResponseConditions[]
xref:ResponseControls[]
xref:ResponseMetrics[]
xref:ResponseProfile[]
xref:ResponseProfileTiming[]
xref:ResponseProfileTimingStats[]
...
For more information, see [Named Parameters and Positional Parameters][section_srh_tlm_n1b].
...
Specifies that prepared statements should be executed automatically as soon as they're created.
...
For more information, see [Auto-Execute][auto-execute].
...
a¦ [markdown]
--
A piece of data supplied by the client that's echoed in the response, if present.
SQL++ is agnostic about the content of this parameter; it's just echoed in the response.
...
{blank}
a¦ xref:RequestCredentials[] array
a¦ [#durability_level]
*durability_level* +
optional
a¦ [markdown]
...
It's included for compatibility with previous versions of Couchbase Server.
--
...
To make a natural language request, you must have a Couchbase Capella user account.
...
[markdown]
--
The Couchbase Capella user account credentials for a natural language request, in the form `username:password`.
...
*natural_config* +
optional
a¦ [.status]##Couchbase Server 8.1##

[markdown]
--
The configuration for a natural language request that talks directly to an LLM provider, instead of using the Couchbase Capella iQ service.
It selects the LLM provider that generates the SQL++ statement and supplies its credentials and options.

Supply either `cred_id` or `api_key`, unless the provider authenticates from the node's ambient environment (for example, Amazon Bedrock via the AWS default credential chain) or a custom `endpoint` is supplied.

This is an alternative to the Capella `natural_cred`/`natural_orgid` parameters. If either `natural_cred` or `natural_orgid` is present, the request is served by the Capella iQ path (and both parameters are then required); `natural_config` is used only when neither is present. To use this parameter, you must also specify the `natural` and `natural_context` parameters.
--

[%hardbreaks]
*Example:* ``
{blank}
a¦ xref:Request_natural_config[]
a¦ [#natural_context]
*natural_context* +
optional
a¦ [.status]##Couchbase Server 8.0##
...
For examples, see [EXECUTE][execute].
...
The query context may be a full path, containing namespace, bucket, and scope; or a relative path, containing just the bucket and scope.
Currently, only the `default` namespace is available.
...
When scan consistency is set to `at_plus`, you must also specify the `scan_vector` parameter for queries using a single keyspace, or the `scan_vectors` parameter for queries using multiple keyspaces.
This is used as a lower bound for the statements in the request.
You can use this setting to implement read-your-own-writes (RYOW).
...
The default transactional scan consistency is RYOW for each statement within the transaction.
If you want to disable RYOW for a statement within the transaction, set `scan_consistency` for that statement to `not_bounded`.
...
a¦ [markdown]
--
Specifies the lower bound vector timestamp for a single keyspace when using `at_plus` scan consistency.

The scan vector may be full or sparse.

* A full scan vector is an array of [`value`, `guard`] entries, giving an entry for every vBucket in the system.

* A sparse scan vector is an object mapping [`value`, `guard`] entries to specific vBuckets.
The name of each property in the object is a vBucket number (a string).
--

[%hardbreaks]
{blank}
a¦ xref:RequestScanVector[]
a¦ [#scan_vectors]
*scan_vectors* +
optional
a¦ [markdown]
--
Specifies the lower bound vector timestamps for multiple keyspaces when using `at_plus` scan consistency.

An object mapping scan vectors to keyspaces.
The name of each property in the object is a keyspace name.
--

[%hardbreaks]
{blank}
a¦ xref:RequestScanVectors[]
a¦ [#scan_wait]
*scan_wait* +
optional
a¦ [markdown]
...
After receiving the scan request, if the indexer is unable to catch up within the `scan_wait` time and start the scan, the indexer aborts with an error and the scan fails.
...
For other statements, it's ignored.
...
If a qualified Search index is available, it's selected for the query.
If none of the available Search indexes are qualified, the available GSI indexes are considered instead.

For more information, see [Flex Indexes][flex-indexes].
...
include::{specDir}definitions/RequestCredentials/definition-before.adoc[opts=optional]

ifdef::show-RequestCredentials[]
[#RequestCredentials]
endif::show-RequestCredentials[]
ifndef::show-RequestCredentials[]
:leveloffset: +1

[discrete#RequestCredentials]
endif::show-RequestCredentials[]
= Credentials

:leveloffset: +1

include::{specDir}definitions/RequestCredentials/definition-begin.adoc[opts=optional]


.icon:brackets-curly[fw] Object
{blank}

//tag::RequestCredentials[]
...
Bucket names may be prefixed with `local:`, and admin names may be prefixed with `admin:`.
--
...
include::{specDir}definitions/RequestCredentials/definition-end.adoc[opts=optional]
...
include::{specDir}definitions/RequestCredentials/definition-after.adoc[opts=optional]

include::{specDir}definitions/Request_natural_config/definition-before.adoc[opts=optional]

ifdef::show-Request_natural_config[]
[#Request_natural_config]
endif::show-Request_natural_config[]
ifndef::show-Request_natural_config[]
:leveloffset: +1

[discrete#Request_natural_config]
endif::show-Request_natural_config[]
= RequestNaturalConfig

:leveloffset: +1

include::{specDir}definitions/Request_natural_config/definition-begin.adoc[opts=optional]


.icon:brackets-curly[fw] Object
{blank}

//tag::Request_natural_config[]
...
a¦ [markdown]
--
The LLM provider. Supported values are `openai`, `bedrock`, `gemini`, and `slm`. Defaults to `openai`. The `slm` provider is for a self-hosted model and always requires `endpoint`.
--
...
a¦ [markdown]
--
The provider-specific model identifier. Defaults to the provider's default model.
--
...
a¦ [markdown]
--
A reference to a stored credential. Supply either `cred_id` or `api_key`.
--
...
a¦ [markdown]
--
An API key for inline authentication. Supply either `api_key` or `cred_id`. Do not expose it in logs.
--
...
a¦ [markdown]
--
A custom endpoint URL for a self-hosted or proxied deployment. Must be permitted by the cluster's URL allow list.
--
...
a¦ [markdown]
--
The provider region. For Amazon Bedrock, this is the AWS region.
--

[%hardbreaks]
{blank}
a¦ String
a¦ *output_token_limit* +
optional
a¦ [markdown]
--
Caps the number of tokens the provider may generate. If omitted, the provider's default applies.
--
...
a¦ [markdown]
--
Whether to run content moderation on the request before generating the statement. Defaults to `true`. Set to `false` for OpenAI-compatible endpoints that do not implement a moderations API (for example, Amazon Bedrock's OpenAI-compatible surface).
--
...
//end::Request_natural_config[]



include::{specDir}definitions/Request_natural_config/definition-end.adoc[opts=optional]
...
include::{specDir}definitions/Request_natural_config/definition-after.adoc[opts=optional]

include::{specDir}definitions/RequestScanVector/definition-before.adoc[opts=optional]

ifdef::show-RequestScanVector[]
[#RequestScanVector]
endif::show-RequestScanVector[]
ifndef::show-RequestScanVector[]
:leveloffset: +1

[discrete#RequestScanVector]
endif::show-RequestScanVector[]
= Scan Vector

:leveloffset: +1

include::{specDir}definitions/RequestScanVector/definition-begin.adoc[opts=optional]

.icon:bars[fw] Composite Schema
{blank}

//tag::RequestScanVector[]
[cols="25,55,20",separator=¦]
|===
...
a¦ [markdown]
--
An array of [`value`, `guard`] entries, giving an entry for every vBucket in the system.
--

[%hardbreaks]
{blank}
a¦ xref:RequestScanVectorFull[]

a¦ or
a¦ [markdown]
--
An object providing entries for specific vBuckets, mapping a vBucket number (a string) to each [`value`, `guard`] entry.
--

[%hardbreaks]
{blank}
a¦ xref:RequestScanVectorSparse[]
|===

//end::RequestScanVector[]

include::{specDir}definitions/RequestScanVector/definition-end.adoc[opts=optional]
...
include::{specDir}definitions/RequestScanVector/definition-after.adoc[opts=optional]

include::{specDir}definitions/RequestScanVectorFull/definition-before.adoc[opts=optional]

ifdef::show-RequestScanVectorFull[]
[#RequestScanVectorFull]
endif::show-RequestScanVectorFull[]
ifndef::show-RequestScanVectorFull[]
:leveloffset: +1

[discrete#RequestScanVectorFull]
endif::show-RequestScanVectorFull[]
= Full Scan Vector

:leveloffset: +1

include::{specDir}definitions/RequestScanVectorFull/definition-begin.adoc[opts=optional]


.icon:brackets[fw] Array
{blank}

//tag::RequestScanVectorFull[]
...
a¦ [markdown]
--
A nested array containing two elements, the `value` and the `guard`.
--
...
*Maximum items:* `2`
{blank}
a¦ xref:RequestScanVectorValueGuard[]

|===

//end::RequestScanVectorFull[]

include::{specDir}definitions/RequestScanVectorFull/definition-end.adoc[opts=optional]
...
include::{specDir}definitions/RequestScanVectorFull/definition-after.adoc[opts=optional]

include::{specDir}definitions/RequestScanVectorSparse/definition-before.adoc[opts=optional]

ifdef::show-RequestScanVectorSparse[]
[#RequestScanVectorSparse]
endif::show-RequestScanVectorSparse[]
ifndef::show-RequestScanVectorSparse[]
:leveloffset: +1

[discrete#RequestScanVectorSparse]
endif::show-RequestScanVectorSparse[]
= Sparse Scan Vector

:leveloffset: +1

include::{specDir}definitions/RequestScanVectorSparse/definition-begin.adoc[opts=optional]


.icon:brackets-curly[fw] Object
{blank}

//tag::RequestScanVectorSparse[]
...
//end::RequestScanVectorSparse[]



include::{specDir}definitions/RequestScanVectorSparse/definition-end.adoc[opts=optional]
...
include::{specDir}definitions/RequestScanVectorSparse/definition-after.adoc[opts=optional]

include::{specDir}definitions/RequestScanVectorValueGuard/definition-before.adoc[opts=optional]

ifdef::show-RequestScanVectorValueGuard[]
[#RequestScanVectorValueGuard]
endif::show-RequestScanVectorValueGuard[]
ifndef::show-RequestScanVectorValueGuard[]
:leveloffset: +1

[discrete#RequestScanVectorValueGuard]
endif::show-RequestScanVectorValueGuard[]
= Value-Guard Entry

:leveloffset: +1

include::{specDir}definitions/RequestScanVectorValueGuard/definition-begin.adoc[opts=optional]


.icon:brackets[fw] Array
{blank}

//tag::RequestScanVectorValueGuard[]
...
a¦ [markdown]
--
The first item is the `value`.
This is a vBucket's sequence number (a JSON number).

The second item is the `guard`.
This is a vBucket's UUID (a string).
--
...
//end::RequestScanVectorValueGuard[]

include::{specDir}definitions/RequestScanVectorValueGuard/definition-end.adoc[opts=optional]
...
include::{specDir}definitions/RequestScanVectorValueGuard/definition-after.adoc[opts=optional]

include::{specDir}definitions/RequestScanVectors/definition-before.adoc[opts=optional]

ifdef::show-RequestScanVectors[]
[#RequestScanVectors]
endif::show-RequestScanVectors[]
ifndef::show-RequestScanVectors[]
:leveloffset: +1

[discrete#RequestScanVectors]
endif::show-RequestScanVectors[]
= Scan Vectors

:leveloffset: +1

include::{specDir}definitions/RequestScanVectors/definition-begin.adoc[opts=optional]


.icon:brackets-curly[fw] Object
{blank}

//tag::RequestScanVectors[]
...
//end::RequestScanVectors[]



include::{specDir}definitions/RequestScanVectors/definition-end.adoc[opts=optional]
...
include::{specDir}definitions/RequestScanVectors/definition-after.adoc[opts=optional]
...
[%hardbreaks]
{blank}
a¦ xref:ResponseConditions[] array
a¦ *warnings* +
optional
a¦ [markdown]
--
An array of 0 or more warning objects.
If a warning occurred during processing of the request, it's represented by a warning object in this list.
--
...
include::{specDir}definitions/ResponseConditions/definition-before.adoc[opts=optional]

ifdef::show-ResponseConditions[]
[#ResponseConditions]
endif::show-ResponseConditions[]
ifndef::show-ResponseConditions[]
:leveloffset: +1

[discrete#ResponseConditions]
endif::show-ResponseConditions[]
= Conditions

:leveloffset: +1

include::{specDir}definitions/ResponseConditions/definition-begin.adoc[opts=optional]


.icon:brackets-curly[fw] Object
{blank}

//tag::ResponseConditions[]
...
//end::ResponseConditions[]



include::{specDir}definitions/ResponseConditions/definition-end.adoc[opts=optional]
...
include::{specDir}definitions/ResponseConditions/definition-after.adoc[opts=optional]

include::{specDir}definitions/ResponseControls/definition-before.adoc[opts=optional]

ifdef::show-ResponseControls[]
[#ResponseControls]
endif::show-ResponseControls[]
ifndef::show-ResponseControls[]
:leveloffset: +1

[discrete#ResponseControls]
endif::show-ResponseControls[]
= Controls

:leveloffset: +1

include::{specDir}definitions/ResponseControls/definition-begin.adoc[opts=optional]


.icon:brackets-curly[fw] Object
{blank}

//tag::ResponseControls[]
...
include::{specDir}definitions/ResponseControls/definition-end.adoc[opts=optional]
...
include::{specDir}definitions/ResponseControls/definition-after.adoc[opts=optional]

include::{specDir}definitions/ResponseMetrics/definition-before.adoc[opts=optional]

ifdef::show-ResponseMetrics[]
[#ResponseMetrics]
endif::show-ResponseMetrics[]
ifndef::show-ResponseMetrics[]
:leveloffset: +1

[discrete#ResponseMetrics]
endif::show-ResponseMetrics[]
= Metrics

:leveloffset: +1

include::{specDir}definitions/ResponseMetrics/definition-begin.adoc[opts=optional]
...
a¦ [markdown]
--
The total time taken for the request &mdash; in other words, the time from when the request was received until the results were returned.
--
...
a¦ [markdown]
--
The time taken for the execution of the request &mdash; in other words, the time from when query execution started until the results were returned.
--
...
{blank}
a¦ Integer (unsigned)
a¦ *naturalLanguageProcessingTime* +
optional
a¦ [markdown]
--
...
include::{specDir}definitions/ResponseMetrics/definition-end.adoc[opts=optional]
...
include::{specDir}definitions/ResponseMetrics/definition-after.adoc[opts=optional]

include::{specDir}definitions/ResponseProfile/definition-before.adoc[opts=optional]

ifdef::show-ResponseProfile[]
[#ResponseProfile]
endif::show-ResponseProfile[]
ifndef::show-ResponseProfile[]
:leveloffset: +1

[discrete#ResponseProfile]
endif::show-ResponseProfile[]
= Profile

:leveloffset: +1

include::{specDir}definitions/ResponseProfile/definition-begin.adoc[opts=optional]
...
include::{specDir}definitions/ResponseProfile/definition-end.adoc[opts=optional]
...
include::{specDir}definitions/ResponseProfile/definition-after.adoc[opts=optional]

include::{specDir}definitions/ResponseProfileTiming/definition-before.adoc[opts=optional]

ifdef::show-ResponseProfileTiming[]
[#ResponseProfileTiming]
endif::show-ResponseProfileTiming[]
ifndef::show-ResponseProfileTiming[]
:leveloffset: +1

[discrete#ResponseProfileTiming]
endif::show-ResponseProfileTiming[]
= Execution Timings

:leveloffset: +1

include::{specDir}definitions/ResponseProfileTiming/definition-begin.adoc[opts=optional]


.icon:brackets-curly[fw] Object
{blank}

//tag::ResponseProfileTiming[]
...
[%hardbreaks]
{blank}
a¦ xref:ResponseProfileTimingStats[]
a¦ *~child* +
optional
a¦ [markdown]
--
...
//end::ResponseProfileTiming[]



include::{specDir}definitions/ResponseProfileTiming/definition-end.adoc[opts=optional]
...
include::{specDir}definitions/ResponseProfileTiming/definition-after.adoc[opts=optional]

include::{specDir}definitions/ResponseProfileTimingStats/definition-before.adoc[opts=optional]

ifdef::show-ResponseProfileTimingStats[]
[#ResponseProfileTimingStats]
endif::show-ResponseProfileTimingStats[]
ifndef::show-ResponseProfileTimingStats[]
:leveloffset: +1

[discrete#ResponseProfileTimingStats]
endif::show-ResponseProfileTimingStats[]
= Statistics

:leveloffset: +1

include::{specDir}definitions/ResponseProfileTimingStats/definition-begin.adoc[opts=optional]


.icon:brackets-curly[fw] Object
{blank}

//tag::ResponseProfileTimingStats[]
...
For index scan, it's time spent waiting for GSI/indexer.

For fetch, it's time spent waiting on the KV store.
--
...
//end::ResponseProfileTimingStats[]



include::{specDir}definitions/ResponseProfileTimingStats/definition-end.adoc[opts=optional]
...
include::{specDir}definitions/ResponseProfileTimingStats/definition-after.adoc[opts=optional]
...
include::{specDir}security/Header/security-scheme-before.adoc[opts=optional]
...
include::{specDir}security/Header/security-scheme-begin.adoc[opts=optional]
...
include::{specDir}security/Header/security-scheme-end.adoc[opts=optional]

:leveloffset: -1

include::{specDir}security/Header/security-scheme-after.adoc[opts=optional]

include::{specDir}security/Parameter/security-scheme-before.adoc[opts=optional]
...
include::{specDir}security/Parameter/security-scheme-begin.adoc[opts=optional]
...
include::{specDir}security/Parameter/security-scheme-end.adoc[opts=optional]

:leveloffset: -1

include::{specDir}security/Parameter/security-scheme-after.adoc[opts=optional]
