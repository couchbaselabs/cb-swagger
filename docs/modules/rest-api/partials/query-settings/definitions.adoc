...
__optional__|The percentage of the `queryNodeQuota` that is dedicated to tracked value content memory across all active requests for every Query node in the cluster.
...
|**queryNumCpus** +
__optional__|The number of CPUs the Query service can use on any Query node in the cluster.
