...
When you change the cluster-level setting, the node-level setting is over-written for all nodes in the cluster. +
**Minimum value** : `0` +
**Maximum value** : `20840448`|integer
|**completed-threshold** +
...
When you change the cluster-level setting, the node-level setting is over-written for all nodes in the cluster.|integer
|**node-quota-val-percent** +
__optional__|The percentage of the `node-quota` that is dedicated to tracked value content memory across all active requests on this node.
(The `memory-quota` setting specifies the maximum amount of memory an individual request may use on this node.)
...
When you change the cluster-level setting, the node-level setting is over-written for all nodes in the cluster. +
**Minimum value** : `0` +
**Maximum value** : `100`|integer
|**num-cpus** +
__optional__|The number of CPUs the Query service can use on this node.
